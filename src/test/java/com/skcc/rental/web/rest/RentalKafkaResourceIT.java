...
class RentalKafkaResourceIT {
    ...
    void setup() {
        ...
        RentalKafkaResource kafkaResource = new RentalKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/rental-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/rental-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
